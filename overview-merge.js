...
function compactStatusBreakdown(){
  ...
  return `<div style="display:grid;grid-template-columns:130px minmax(0,1fr);gap:14px;align-items:center;min-height:230px"><div style="display:grid;place-items:center"><div title="Status distribution" style="width:128px;aspect-ratio:1;border-radius:50%;background:${gradient};display:grid;place-items:center;box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--border) 55%,transparent)"><div style="width:72px;aspect-ratio:1;border-radius:50%;background:var(--surface);display:grid;place-items:center;text-align:center;box-shadow:0 0 0 1px var(--border)"><div><strong style="display:block;font:600 1.3rem/1 var(--serif)">${total}</strong><span style="display:block;margin-top:4px;font:400 .52rem var(--mono);text-transform:uppercase;letter-spacing:.05em;color:var(--ink-3)">records</span></div></div></div></div><div class="legend" style="margin-top:0;display:grid;grid-template-columns:1fr;gap:5px;min-width:0">${entries.map(([k,v],i)=>{const pct=total?Math.round(v/total*100):0,color=STATUS_COLORS[k]||['var(--accent)','var(--prepared)','var(--good)','var(--warn)','var(--risk)','var(--ink-3)'][i%6];return `<span title="${esc(k)}"><i style="background:${color};border-radius:50%"></i><span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(shortStatus(k))}</span><strong style="margin-left:auto;color:var(--ink);font:600 .64rem var(--mono)">${pct}%</strong></span>`}).join('')}</div></div>`;
}
