...
function preparedDeadlineSort(a,b){const da=dateFrom(a.Deadline),db=dateFrom(b.Deadline);if(!da&&!db)return String(a.Company||'').localeCompare(String(b.Company||''));if(!da)return-1;if(!db)return 1;return db-da||String(a.Company||'').localeCompare(String(b.Company||''))}
function deadlineSort(a,b){const aMissing=a.Status==='Prepared'&&!dateFrom(a.Deadline),bMissing=b.Status==='Prepared'&&!dateFrom(b.Deadline);if(aMissing!==bMissing)return aMissing?-1:1;const da=dateFrom(effectiveDeadline(a)),db=dateFrom(effectiveDeadline(b));if(!da&&!db)return String(a.Company||'').localeCompare(String(b.Company||''));if(!da)return 1;if(!db)return-1;return db-da||String(a.Company||'').localeCompare(String(b.Company||''))}
